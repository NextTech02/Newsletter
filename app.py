...
st.set_page_config(
    page_title="Newsletter Template",
    page_icon="📰",
    layout="wide"
)
...
# Título principal
st.title("📰 Template de Newsletter")
st.markdown("---")
...
with st.sidebar:
    ...
    # Toggle para tema do site
    st.subheader("🌙 Tema do Site")
    dark_mode = st.toggle(
        "Modo Escuro",
        value=st.session_state.site_dark_mode,
        help="Alternar entre tema claro e escuro do site"
    )
    if dark_mode != st.session_state.site_dark_mode:
        st.session_state.site_dark_mode = dark_mode
        st.rerun()
    ...
    theme = st.selectbox(
        "Tema da Newsletter",
        ["Claro", "Escuro", "Azul", "Verde", "Roxo", "Federación Poker"],
        index=["Claro", "Escuro", "Azul", "Verde", "Roxo", "Federación Poker"].index(st.session_state.selected_theme)
    )
    st.session_state.selected_theme = theme
    
    # Configurações especiais para tema FCP
    if theme == "Federación Poker":
        col1, col2 = st.columns(2)
        ...
        with col1:
            compact_mode = st.checkbox(
                "📦 Modo Ultra-Compacto",
                value=True,
                help="Reduz drasticamente o tamanho do HTML para evitar corte do Gmail",
        ...
        with col2:
            layout_style = st.selectbox(
                "🎨 Estilo de Layout",
                ["Moderno", "Minimalista", "Corporativo"],
                index=0,
                help="Escolha o estilo visual da newsletter",
                key="layout_style_selectbox"
            )
            
            # Mostrar descrição do estilo selecionado
            style_descriptions = {
                "Moderno": "🎨 Cards com sombras, gradientes e design contemporâneo",
                "Minimalista": "⚪ Design limpo com bordas coloridas e espaçamento generoso", 
                "Corporativo": "💼 Estilo profissional com fundos cinza e bordas definidas"
            }
            st.caption(style_descriptions[layout_style])
    else:
        compact_mode = False
    ...
    if not supabase_url or not supabase_key:
        st.info("Configure o Supabase para gerenciar leads")
        
        with st.expander("⚙️ Configurar Supabase", expanded=False):
            st.markdown("**1. Crie um arquivo `.env` na raiz do projeto:**")
            st.code("""
SUPABASE_URL=sua_url_do_supabase
SUPABASE_KEY=sua_chave_do_supabase
            """, language="bash")
            
            st.markdown("**2. Ou configure manualmente:**")
            url = st.text_input("URL do Supabase", placeholder="https://seu-projeto.supabase.co")
            key = st.text_input("Chave do Supabase", type="password", placeholder="sua-chave-aqui")
            
            if st.button("🔗 Conectar", type="primary"):
                if url and key:
                    st.session_state.supabase_url = url
                    st.session_state.supabase_key = key
                    st.session_state.supabase_configured = True
                    st.success("✅ Conectado ao Supabase!")
                    st.rerun()
                else:
                    st.error("⚠️ Preencha URL e Chave do Supabase")
    else:
    ...
    if not email_config['smtp_username'] or not email_config['smtp_password']:
        st.info("Configure o email para enviar newsletters")
...
SMTP_PASSWORD=sua_senha_de_aplicativo
            ...
            st.markdown("**2. Ou configure manualmente:**")
            st.markdown("**⚠️ IMPORTANTE:** Para Gmail, use uma senha de aplicativo, não sua senha normal!")
            st.markdown("**Como obter senha de aplicativo:**")
            st.markdown("1. Ative a verificação em duas etapas no Gmail")
            st.markdown("2. Vá em 'Senhas de aplicativo'")
            st.markdown("3. Gere uma senha para 'Newsletter App'")
            st.markdown("4. Use essa senha aqui (não sua senha normal)")
            
            username = st.text_input("Email do Gmail", placeholder="seu_email@gmail.com")
            password = st.text_input("Senha de Aplicativo", type="password", placeholder="sua-senha-de-aplicativo")
            
            if st.button("📧 Configurar Email", type="primary"):
                if username and password:
                    # Salvar nas variáveis de ambiente da sessão
                    os.environ['SMTP_USERNAME'] = username
                    os.environ['SMTP_PASSWORD'] = password
                    st.success("✅ Email configurado! Teste a conexão na aba 'Enviar'")
                    st.rerun()
                else:
                    st.error("⚠️ Preencha email e senha de aplicativo")
    else:
        st.success("✅ Email configurado")
        
        # Mostrar email configurado (mascarado)
        masked_email = email_config['smtp_username'][:3] + "***@" + email_config['smtp_username'].split('@')[1]
        st.caption(f"📧 {masked_email}")
    
    st.markdown("---")
    
    # Botão para limpar tudo
    if st.button("🗑️ Limpar Newsletter", type="secondary"):
        st.session_state.newsletter_title = ""
        st.session_state.news_items = [{"title": "", "content": ""}]
        st.rerun()
...
with tab1:
    ...
    with col1:
        st.header("✏️ Editor da Newsletter")
    ...
    with col2:
        st.header("👁️ Preview da Newsletter")
        ...
        if newsletter_title or any(item["title"] or item["content"] for item in st.session_state.news_items):
            ...
            if size_info['status'] == "MUITO GRANDE":
                st.warning("⚠️ HTML muito grande! Use o modo ultra-compacto ou crie HTML mínimo.")
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("🎨 HTML Mínimo", key="minimal_html_btn"):
                        html_content = create_minimal_html(newsletter_title, st.session_state.news_items, newsletter_date)
                        st.success("✅ HTML mínimo com design profissional criado!")
                with col2:
                    if st.button("⚡ Ultra-Compacto", key="ultra_compact_btn"):
                        html_content = create_ultra_compact_html(newsletter_title, st.session_state.news_items, newsletter_date)
                        st.success("✅ HTML ultra-compacto com design elegante criado!")
    ...
    # Seção de exportação dentro da aba Editor
    st.markdown("---")
    st.header("📤 Exportar Newsletter")
    
    # Dicas para evitar corte do Gmail
    with st.expander("💡 Dicas para evitar corte do Gmail", expanded=False):
        st.markdown("""
        **Para evitar que o Gmail corte sua newsletter:**
        
        1. **Modo Ultra-Compacto** ✅ (Design bonito + ~5KB)
        2. **HTML Mínimo** ✅ (Design profissional + ~10KB) 
        3. **Ultra-Compacto** ✅ (Design elegante + ~3KB)
        4. **Evite logo base64** ✅ (Remove ~400KB)
        5. **Use tabelas simples** ✅
        6. **Minimize CSS** ✅
        7. **Teste sempre antes de enviar** ✅
        
        **🎨 Design Preservado:**
        - ✅ Paleta de cores da FCP (Vermelho #d32f2f, Azul #1976d2, Dourado #ff9800)
        ...
        **Dicas de Design e Otimização:**
        - **CTA claro**: Botões com cores contrastantes
        - **Hierarquia visual**: Títulos maiores que subtítulos
        - **Espaçamento**: Use padding e margin consistentes
        - **Cores**: Máximo 3 cores principais
        - **Tipografia**: Fontes legíveis (Arial, Helvetica)
        - **Engajamento**: Links para redes sociais e site
        """)
    ...
    if has_content:
        # Layout com mais espaçamento e melhor distribuição
        ...
        # Layout melhorado com três colunas iguais e centralizadas
        col1, col2, col3 = st.columns([1, 1, 1], gap="large")
        
        # HTML Export
        with col1:
            st.markdown("""
            <div style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 10px; border: 1px solid #e9ecef; height: 200px; display: flex; flex-direction: column; justify-content: center;">
                <h3 style="margin: 0 0 15px 0; color: #333;">📄 HTML</h3>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.download_button(
                label="⬇️ Baixar HTML",
                data=html_content,
                file_name=f"newsletter_{newsletter_date.strftime('%Y%m%d')}.html",
                mime="text/html",
                type="primary",
                width='stretch'
            )
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            st.caption("💡 Para envio por email ou publicação web")
        
        # Markdown Export
        with col2:
            st.markdown("""
            <div style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 10px; border: 1px solid #e9ecef; height: 200px; display: flex; flex-direction: column; justify-content: center;">
                <h3 style="margin: 0 0 15px 0; color: #333;">📝 Markdown</h3>
            </div>
            """, unsafe_allow_html=True)
            
            st.markdown("<br>", unsafe_allow_html=True)
            
            markdown_content = generate_markdown_newsletter(newsletter_title, st.session_state.news_items, newsletter_date)
            st.download_button(
                label="⬇️ Baixar Markdown",
                data=markdown_content,
                file_name=f"newsletter_{newsletter_date.strftime('%Y%m%d')}.md",
                mime="text/markdown",
                type="primary",
                width='stretch'
            )
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            st.caption("💡 Para documentação ou plataformas que suportam Markdown")
        
        # Text Export
        with col3:
            st.markdown("""
            <div style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 10px; border: 1px solid #e9ecef; height: 200px; display: flex; flex-direction: column; justify-content: center;">
                <h3 style="margin: 0 0 15px 0; color: #333;">📋 Texto Simples</h3>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.download_button(
                ...
                data=text_content,
                file_name=f"newsletter_{newsletter_date.strftime('%Y%m%d')}.txt",
                mime="text/plain",
                type="primary",
                width='stretch'
            )
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            st.caption("💡 Para envio por mensagens de texto")
        
        st.markdown("<br><br><br>", unsafe_allow_html=True)  # Espaço extra no final
        
        # Área de texto para copiar manualmente
        st.markdown("---")
        st.subheader("📋 Copiar Texto Manualmente")
        st.text_area(
            "Texto da Newsletter (copie e cole):", 
            value=text_content, 
            height=200,
            help="Use esta área para copiar o texto da newsletter manualmente"
        )
    else:
        st.warning("⚠️ Adicione um título e pelo menos uma notícia para poder exportar a newsletter")
...
with tab2:
    ...
    if not supabase_url or not supabase_key:
        st.warning("⚠️ Configure o Supabase na sidebar para gerenciar leads")
    else:
...
with tab3:
    ...
    if not has_content:
        st.warning("⚠️ Crie uma newsletter na aba 'Editor' antes de enviar")
    else:
        supabase_url, supabase_key = get_supabase_config()
        
        if not supabase_url or not supabase_key:
            st.warning("⚠️ Configure o Supabase na sidebar para enviar newsletter")
        else:
            # Conectar ao Supabase
            if supabase_manager.connect():
                st.success("✅ Conectado ao Supabase")
                ...
                if not subscribed_leads:
                    st.warning("⚠️ Nenhum lead inscrito encontrado. Adicione leads na aba 'Leads'")
                else:
                    ...
                    with col1:
                        ...
                        if test_email and st.button("🧪 Enviar Teste", type="secondary", width='stretch'):
                            if email_manager.test_connection():
                                ...
                                with st.expander("🔍 Preview do HTML (Debug)", expanded=False):
